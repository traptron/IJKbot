...
перенесён на `/nav2/manual_goal_pose`. Скорости проходят по цепочке
controller/behaviors → `/cmd_vel_nav` → smoother → `/trial/cmd_vel` → планер
→ `/cmd_vel` → драйвер. Планер пропускает их только во время движения;
при удержании, завершении и E-STOP выдаёт ноль. Другие издатели `/cmd_vel`
(например, teleop или старый экземпляр GUI) должны быть остановлены.
Не запускайте второй экземпляр Nav2.
...
Полный запуск системы теперь включает планер по умолчанию:

```bash
ros2 launch ijkbot_bringup system.launch.py mock_hardware:=true
```

Для реального запуска задайте `mock_hardware:=false`. Этот запуск заменяет
совместный запуск `robot.launch.py` и `trial.launch.py`.
`trial_mode:=false` выбирает обычный Nav2 без планера.
Координаты возврата берутся из `initial_x`, `initial_y`, `initial_yaw`, как и
начальное преобразование map→odom. Потеря /odom при движении отменяет миссию.

Актуальный GUI (`mission_sm`, параметр `trial_mode:=true` по умолчанию)
в реальном режиме отображает `/trial/state` и отправляет одну цель при старте;
поиск/QR и второй автомат движения не запускаются. Пауза отменяет испытание;
сброс E-STOP разрешает новый запуск кнопкой Старт. Mock GUI не отправляет
команды приводам. Для прежнего самостоятельного автомата нужен
`trial_mode:=false` и соответствующий запуск системы без планера.
Текстовый топик `/mission/judge_task` в режиме испытания обрабатывает только
`llm_client`; GUI может отдельно интерпретировать текст по своей кнопке.
