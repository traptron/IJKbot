...
1. **Разработка пакета `sts3215_driver` (C++ / ROS 2 Jazzy)**:
   ...
   - Разработать ноду `diff_drive_node`:
     ...
     - Пересчет скоростей по дифференциальной кинематике ($D = 75\text{ мм}$, $L = 140\text{ мм}$).
...
- [ ] При обрыве связи или прерывании публикации `/cmd_vel` моторы останавливаются в течение 200 мс.
...
1. **Развертывание LLM на ноутбуке с RTX 5060**:
   - Установка и настройка Ollama с моделью `qwen2.5:7b-instruct-q4_K_M`.
   - Проверка потребления видеопамяти (должно укладываться в ~4.5–5.0 ГБ из доступных 8 ГБ).

2. **Разработка пакета `ijkbot_brain`**:
   - `ijkbot_brain/llm_client.py`:
     - Формирование системного промпта с жестко заданной Pydantic/JSON-схемой и списком допустимых ориентиров полигона (`smoke_tower`, `panel_house`, `bridges`, `tanker_truck`, `fallen_tree`, `car_jam`, `debris_pvc`).
     - Запрос к Ollama API в режиме `format: "json"`.
     - Парсинг и валидация ответа: извлечение ID ориентира, приоритета поиска и текстового обоснования для судей.
   - Тестовый скрипт с набором типовых судейских формулировок на русском языке для валидации 100% точности распознавания.

### Критерии приемки:
- [ ] Время обработки текста задания моделью составляет менее 2 секунд на RTX 5060.
- [ ] Любое валидное описание объекта из регламента однозначно мапится в правильный `target_landmark_id`.
- [ ] Ответ всегда возвращается в строгом JSON-формате без лишнего текста.
