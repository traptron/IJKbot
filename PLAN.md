...
Дополнение — координатное испытание: реализован отдельный `trial_planner`
(`/goal_pose` → NavigateToPose → 5 секунд неподвижности по /odom → старт
map (0.4, 0.4)). Запуск: `ros2 launch ijkbot_nav2 trial.launch.py`.
Есть подтверждение action-результата, отмена при E-STOP, общий таймаут 300 с
и защита от новых целей во время миссии. Эти изменения относятся к новому
планеру; ограничения старого `mission_sm` выше сохраняются. Поиск человека и
QR в данный сценарий не входят. Физическая приёмка полного цикла не выполнена.

Интеграция испытания: `system.launch.py` выбирает trial по умолчанию, обычный
Nav2 доступен через `trial_mode:=false`. Скорости Nav2 проходят через планер,
GUI в реальном trial-режиме отображает его статусы вместо исполнения старого
автомата. Стартовая поза и возврат используют общие launch-параметры.
