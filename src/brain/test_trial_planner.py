...
from brain.trial_planner import TrialPlanner
...
def test_success_waits_five_seconds_then_returns_and_completes():
    ...
    with patch('brain.trial_planner.time.monotonic') as clock:
...
def test_no_odom_does_not_start_return():
    node = planner()
    node.state = 'HOLDING'
    with patch('brain.trial_planner.time.monotonic', return_value=20.0):
        node.tick()
    node.send.assert_not_called()
...
def test_motion_and_stale_odom_reset_hold():
    ...
    with patch('brain.trial_planner.time.monotonic', return_value=5.0):
        node.on_odom(moving)
        node.tick()
    assert node.stationary_since is None
    node.stationary = True
    node.stationary_since = 1.0
    with patch('brain.trial_planner.time.monotonic', return_value=6.0):
        node.tick()
    assert node.stationary_since is None
    node.send.assert_not_called()
...
def test_timeout_cancels_navigation():
    node = planner()
    node.handle = Mock()
    with patch('brain.trial_planner.time.monotonic', return_value=301.0):
        node.tick()
    assert node.state == 'FAILED'
