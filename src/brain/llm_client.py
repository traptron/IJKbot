...
MAP_OBJECT_DETAILS: Dict[str, Dict[str, Any]] = {
    ...
    "yellow_building_debris": {"name_ru": "Обломки жёлтого здания", "aliases": ["обломки жёлтого здания", "обломки желтого здания"]},
...
STATIC_TARGET_IDS = ["yellow_building", "parking", "yellow_building_debris", "blue_building", "river", "bridges"]
STATIC_PATTERNS = {
...
def load_arena(path: Optional[str] = None) -> Dict[str, Any]:
    ...
    if path is None:
        ...
        if source.is_file():
            ...
            path = str(Path(get_package_share_directory('brain')) / 'config/arena.json')
...
class LLMClient:
    ...
    def fallback_heuristic_parse(self, text: str) -> CommandInterpretation:
        ...
        """
        if re.search(r"\bобломк\w*\s+желт\w*\s+здани\w*\b", text.lower().replace('ё', 'е')):
            raise ValueError("Ориентир «обломки жёлтого здания» требует распознавания локальной LLM.")
