...
class TestQrConfirmation(unittest.TestCase):
    ...
    def test_fallback_to_standard_qr_detector(self):
        ...
        with patch('vision.qr_decoder.get_wechat_detector', return_value=None):
            result = decode_jpeg(bytes(jpeg))
            self.assertIsNotNone(result)
    ...
    def test_wechat_multi_candidate_with_empty_first(self):
        ...
        with patch('vision.qr_decoder.get_wechat_detector', return_value=mock_wechat), \
             patch('vision.qr_decoder.get_fallback_detector', return_value=None):
            result = decode_jpeg(bytes(jpeg))
            self.assertIsNotNone(result)
