...
from vision.qr_decoder import QrConfirmation, QrDetection
from vision.qr_reader_node import QrReaderNode
...
def frame(node, stamp, now, result):
    ...
    with patch('vision.qr_reader_node.time.monotonic', return_value=now), \
            patch('vision.qr_reader_node.decode_jpeg', return_value=result):
...
def test_corrupt_frame_breaks_confirmation_and_silence_clears_detection():
    ...
    with patch('vision.qr_reader_node.time.monotonic', return_value=10.2), \
            patch('vision.qr_reader_node.decode_jpeg', side_effect=ValueError('bad')):
        QrReaderNode._image_callback(node, CompressedImage())
    frame(node, 4, 10.3, result)
    assert node._status_publisher.publish.call_count == 0
    with patch('vision.qr_reader_node.time.monotonic', return_value=12):
...
def test_photo_trigger_bypasses_state_filter():
    ...
    # Trigger fired
    with patch('vision.qr_reader_node.time.monotonic', return_value=10.0), \
            patch('vision.qr_reader_node.decode_jpeg', return_value=result):
...
def test_trigger_forces_processing_on_identical_stamp():
    ...
    with patch('vision.qr_reader_node.time.monotonic', return_value=1.0), \
         patch('vision.qr_reader_node.decode_jpeg', return_value=None):
        QrReaderNode._image_callback(node, message)
    ...
    with patch('vision.qr_reader_node.time.monotonic', return_value=1.5), \
         patch('vision.qr_reader_node.decode_jpeg', return_value=result):
        QrReaderNode._trigger_callback(node, Bool(data=True))
